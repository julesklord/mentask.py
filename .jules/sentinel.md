...
**Vulnerability:** The `web_fetch` function in `src/mentask/tools/web_tools.py` directly uses `urllib.request.urlopen` with user-supplied URLs without validating the URL scheme.
...
**Vulnerability:** In `src/mentask/core/config_manager.py`, the `save_settings` method failed to remove sensitive API keys from the configuration dictionary if storing them in the system keyring encountered an exception. This resulted in the plaintext key being persisted to the `settings.json` file.

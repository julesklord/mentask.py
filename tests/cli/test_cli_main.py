...
def test_run_chatbot_legacy():
    with patch("askgem.cli.main.argparse.ArgumentParser.parse_args") as mock_parse_args, patch(
        "askgem.agent.chat.ChatAgent"
    ) as mock_agent_class, patch("askgem.cli.console.console") as mock_console, patch(
        "asyncio.run"
    ) as mock_asyncio_run, patch("askgem.core.i18n.get_current_language", return_value="en"):
        mock_args = MagicMock()
...
def test_run_chatbot_dashboard():
    with patch("askgem.cli.main.argparse.ArgumentParser.parse_args") as mock_parse_args, patch(
        ...
        with patch.dict("sys.modules", {"askgem.cli.dashboard": fake_module, "askgem.cli.main.dashboard": fake_module}):
            run_chatbot()
...
def test_run_chatbot_dashboard_fallback():
    with patch("askgem.cli.main.argparse.ArgumentParser.parse_args") as mock_parse_args, patch(
        ...
        mock_console.print.assert_called_with(
            "[warning]Dashboard not implemented yet. Falling back to --legacy...[/warning]"
        )
        mock_asyncio_run.assert_called_once_with(mock_instance.start())

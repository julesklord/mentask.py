...
askgem is a powerful, autonomous command-line interface (CLI) client for interacting with Google's Gemini AI models. Designed for terminal-based development, it now comes equipped with agentic tools that allow it to read, write, and execute commands directly on your machine.

![askgem Logo](docs/askgem.webp)

## ✨ Key Features (v2.0 Redesign)

*   **⚡ Autonomous Agentic Engine:** askgem integrates natively with `google-genai>=0.2.0`, enabling multi-step reasoning, filesystem exploration (`list_directory`, `read_file`), and autonomous actions (`edit_file`, `execute_bash`).
...
*   **🏠 Centralized Configuration:** Easily manage settings via `~/.askgem/settings.json`.
*   **📚 Smart Context Windows:** Sessions are persisted automatically per model (`~/.askgem/history/`). Loads old chat logs utilizing dynamic token "Rolling Windows" to conserve API costs.
...
Since askgem directly manipulates your filesystem during development requests, it is strongly recommended to isolate dependencies properly or run it in a container if working on highly sensitive tasks.
